...
//
import { RuleDefinition, TEST } from './model';
...
export class ProxyManager {
    ...
    private createConfigFileArguments() {
        ...
        const defaultName = TEST.isDryRun() ? "test" : "global";
    ...
    purge(rules: Array<RuleDefinition>) {
        fs.exists(this.engine.certificatesFolder, exists => {
            ...
            fs.readdir(this.engine.certificatesFolder, (err, folders) => {
                ...
                let domainNames = rules.map(d => { let dn = d.tlsDomain.toLowerCase(); if (dn.startsWith("*.*")) dn = dn.substr(2); return dn; });
